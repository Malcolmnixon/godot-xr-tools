...
func pick_up(by: Node3D) -> void:
	...
	if _grab_driver:
		...
		# Add the second hand to the existing grab
		if second_hand_grab != SecondHandGrab.SWAP:
			# Grab the object
			var by_grab_point := _get_grab_point(by, true)
			var grab := Grab.new(grabber, self, by_grab_point, true)
			_grab_driver.set_secondary(grab)
			return
	...
	var by_grab_point := _get_grab_point(by, false)
...
func let_go(by: Node3D, p_linear_velocity: Vector3, p_angular_velocity: Vector3) -> void:
	...
	if _grab_driver.primary:
		...
		if is_instance_valid(_grab_driver.primary.hand_point):
			...
			var new_grab_point := _get_grab_point(_grab_driver.primary.by, false)
